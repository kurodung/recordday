...
// GET /api/ir-reports (list all reports, with optional filters)
router.get("/ir-reports", async (req, res) => {
  try {
    const { start, end, shift } = req.query;
    let sql = `
      SELECT 
        id, date AS report_date, shift, wardname, subward,
        vas, nonvas, consult, opd,
    ...
    console.error("GET /ir-reports error:", e);
    res.status(500).json({ message: "Server error" });
  }
});

// GET /api/ir-report?date=...&shift=...&wardname=...
router.get("/ir-report", async (req, res) => {
  try {
    ...
    console.error("GET /ir-report error:", e);
    res.status(500).json({ message: "Server error" });
  }
});

// POST /api/ir-report
router.post("/ir-report", async (req, res) => {
  try {
    const {
      username, wardname, subward = null, date, shift,
      vas = 0, nonvas = 0, consult = 0, opd = 0,
      rn = 0, pn = 0, na = 0,
    ...
    console.error("POST /ir-report error:", e);
    res.status(500).json({ message: "Server error" });
  }
});

// PUT /api/ir-report/:id
router.put("/ir-report/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const allowed = [
      "username","wardname","subward","date","shift",
    ...
    params.push(id);
    const [result] = await db.query(
      `UPDATE ir_reports SET ${fields.join(", ")} WHERE id = ?`,
      params
    );
    ...
    console.error("PUT /ir-report/:id error:", e);
    res.status(500).json({ message: "Server error" });
  }
});

// GET /api/ir-report/list
// ใช้ใน Dashboard ดึงรายการตามช่วงวันที่/เวร
router.get("/ir-report/list", async (req, res) => {
  try {
    const { start, end, shift } = req.query;
    let sql = `
      SELECT 
    ...
    console.error("GET /ir-report/list error:", e);
    res.status(500).json({ message: "Server error" });
  }
});
