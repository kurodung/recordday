...
export default function Endopage({ username, wardname, selectedDate, shift }) {
    ...
    const fetchExistingData = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE}/api/endo-report?${queryParams.toString()}`,
          {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (wardname.toLowerCase() === "admin") {
        alert("Admin ไม่สามารถบันทึก endo report ได้");
        return;
      }
      ...
      const url = formData.id
        ? `${API_BASE}/api/endo-report/${formData.id}`
        : `${API_BASE}/api/endo-report`;
      ...
      if (!res.ok) {
        console.error("POST/PUT /endo-report failed:", res.status, json || text);
      ...
      <div className="form-section">
        <div className="flex-grid">
          <div className="form-column">
            <div className="section-header">ซับซ้อน</div>
            {renderInput("", "complex")}
          </div>
          <div className="form-column">
            <div className="section-header">Upper GI</div>
            {renderInput("", "upgi")}
          </div>
          <div className="form-column">
            <div className="section-header">Lower GI</div>
            {renderInput("", "lowgi")}
          </div>
          <div className="form-column">
            <div className="section-header">chest</div>
            {renderInput("", "chest")}
          </div>
