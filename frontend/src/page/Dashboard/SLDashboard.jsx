...
export default function SLDashboard() {
    ...
    (async () => {
      try {
        ...
        const url = qs.toString()
          ? `${API_BASE}/api/sl-report/list?${qs.toString()}`
          : `${API_BASE}/api/sl-report/list`;
        ...
        if (!res.ok) throw new Error("โหลด sl reports ไม่สำเร็จ");
  ...
  const renderCards = () => {
    const cards = [
      ...
      { label: "Sleep test", value: fmt(summary.sleep) },
      { label: "Titrate CPAP", value: fmt(summary.cpap) },
  ...
  if (loading) {
    return (
      <div className={styles.loadingContainer}>
        ...
        <span className={styles.loadingText}>กำลังโหลดข้อมูล SL...</span>
      </div>
    );
  }
  ...
  return (
    <div className={styles.dashboardContainer}>
      ...
      <div className={styles.dashboardHeader}>
        <h1 className={styles.dashboardTitle}>Dashboard Sleep lab</h1>
        <p className={styles.dashboardSubtitle}>
          สรุปบริการ (Sleep test, Titrate CPAP, Consult, OPD clinic)
          {rows.length > 0 && (
          ...
          <>
            <div className={stylesmain.tableScroll}>
              <TableBox
                ...
                headers={["วันที่", "เวร", "Sleep test", "cpap", "Consult", "OPD clinic"]}
                rows={pageRows.map((r) => [
