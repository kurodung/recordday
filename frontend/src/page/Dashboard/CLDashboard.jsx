...
export default function CLDashboard() {
    ...
    (async () => {
      try {
        ...
        const url = qs.toString()
          ? `${API_BASE}/api/cl-report/list?${qs.toString()}`
          : `${API_BASE}/api/cl-report/list`;
        ...
        if (!res.ok) throw new Error("โหลด CL reports ไม่สำเร็จ");
  ...
  const renderCards = () => {
    const cards = [
      ...
      { label: "PCI", value: fmt(summary.pci) },
      { label: "CAG", value: fmt(summary.cag) },
      { label: "Fast Track", value: fmt(summary.fasttrack) },
      { label: "นักเทคโนโลยีหัวใจ", value: fmt(summary.tech) },
    ];
    return (
  ...
  return (
    <div className={styles.dashboardContainer}>
      ...
      <div className={styles.dashboardHeader}>
        <h1 className={styles.dashboardTitle}>❤️ CL Dashboard (Cath Lab)</h1>
        <p className={styles.dashboardSubtitle}>
          สรุปบริการ Cath Lab (PCI, CAG, Fast Track)
          {rows.length > 0 && (
          ...
          <>
            <div className={stylesmain.tableScroll}>
              <TableBox
                ...
                headers={["วันที่", "เวร", "PCI", "CAG", "Fast Track"]}
                rows={pageRows.map((r) => [
