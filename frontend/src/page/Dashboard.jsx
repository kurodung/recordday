...
//คิดว่าต้องเปลี่ยนนิดหน่อยตรง ศัลยกรรมหญิง 2 - ๊Uro เพราะศัลย์ 1 มันปกติ
const NORMAL_WARDS = [
  "หลังคลอด - ผู้ใหญ่",
  "นรีเวชกรรม", 
  "SNB - SNB",
...
const Semi_ICU = [
  "SNB - NICU",
  "อายุรกรรมหญิง 3 - Semi ICU", // เผื่อบางฐานข้อมูลใช้ชื่อสั้น
...
const Newborn = [
  "หลังคลอด - ทารก",
  "หลังคลอด - SNB", // เผื่อบางฐานข้อมูลใช้ชื่อสั้น
  "เฉลิมฯ 2 - ทารก",
  "เฉลิมฯ 2 - SNB",
  "เฉลิมฯ 3 - ทารก",
  "เฉลิมฯ 3 - SNB",
...
const { allRemain, specialRemain, specialFound } = useMemo(() => {
  ...
  // แยก SPECIAL_WARDS เป็น "เดี่ยว" กับ "คู่ (ward-subward)"
  const SINGLE = new Set();
  const COMBO  = new Set(); // เก็บเป็น "ward|subward"
  ...
  if (!unifiedRows?.length) return { allRemain: 0, specialRemain: 0, specialFound: false };

  const rows = unifiedRows.filter((r) => !isRollup(r));
  if (!rows.length) {
    const R = unifiedRows.find(isRollup);
    return { allRemain: R ? remainOf(R) : 0, specialRemain: 0, specialFound: false };
  }
  ...
  for (const r of rows) {
    ...
    if (s) {
      if (COMBO.has(`${w}|${s}`)) { sp += rm; found = true; }
    } else {
      if (SINGLE.has(w)) { sp += rm; found = true; }
    }
  }

  return { allRemain: all, specialRemain: sp, specialFound: found };
}, [unifiedRows, SPECIAL_WARDS]);
            ...
            <table style={tableStyle}>
              ...
              <tbody>
                <tr>
                  ...
                  <td style={tdStyle}>{specialFound ? fmt(specialRemain) : "-"}</td>
                </tr>
              </tbody>
            </table>
