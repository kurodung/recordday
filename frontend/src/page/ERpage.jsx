...
export default function ERpage({ username, wardname, selectedDate, shift }) {
    ...
    const fetchExistingData = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE}/api/er-report?${queryParams.toString()}`,
          {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (wardname.toLowerCase() === "admin") {
        alert("Admin ไม่สามารถบันทึก er report ได้");
        return;
      }
      ...
      const url = formData.id
        ? `${API_BASE}/api/er-report/${formData.id}`
        : `${API_BASE}/api/er-report`;
      ...
      if (!res.ok) {
        console.error("POST/PUT /er-report failed:", res.status, json || text);
  ...
  return (
    <div className="form-container" ref={formRef}>
        ...
        <div className="flex-grid">
          <div className="form-column">
            ...
            <div className="horizontal-inputs">
              ...
              {renderInput("รอตรวจ:", "wait_treat")}
              {renderInput("ไม่รอ:", "not_wait")}
              {renderInput("ปฏิเสธรักษา:", "deny_treat")}
              {renderInput("ส่ง OPD:", "opd")}
              {renderInput("refer out:", "refer_out")}
              {renderInput("refer back:", "refer_back")}
              {renderInput("เสียชีวิต:", "death")}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="form-section">
        <div className="flex-grid">
          <div className="form-column">
            <div className="section-header">ระดับความรุนแรง</div>
            <div className="horizontal-inputs">
              {renderInput("R:", "r")}
              {renderInput("E:", "e")}
              {renderInput("U:", "u")}
          ...
          <div className="form-column">
            <div className="section-header">EMS</div>
            {renderInput("", "ems")}
          </div>
          <div className="form-column">
            <div className="section-header">นิติเวช</div>
            {renderInput("", "forensic")}
          </div>
          <div className="form-column">
            <div className="section-header">OU</div>
            {renderInput("", "ou")}
          </div>
          <div className="form-column">
            <div className="section-header">ARI</div>
            {renderInput("", "ari")}
          </div>
          <div className="form-column">
            <div className="section-header">PUI</div>
            {renderInput("", "pui")}
          </div>
          <div className="form-column">
            <div className="section-header">swab</div>
            {renderInput("", "sawb")}
          </div>
          <div className="form-column">
      ...
      <div className="form-section">
        <div className="flex-grid">
          <div className="form-column">
            <div className="section-header">Fast Track</div>
            <div className="horizontal-inputs">
              {renderInput("Stroke:", "stroke")}
              {renderInput("STEMI:", "stemi")}
              {renderInput("Sepsis:", "sepsis")}
              {renderInput("Trauma:", "trauma")}
              {renderInput("Ectopic:", "ectopic")}
              {renderInput("Fast Pass:", "pass")}
            </div>
          </div>
